...
// 添加商品（支持 FormData 和 JSON）
app.post('/api/admin/products', upload.none(), (req, res) => {
  const name = req.body.name;
  const price = req.body.price;
  const description = req.body.description || '';
  const stock = req.body.stock || 999;
...
// 上传收款码（支持URL或Base64）
app.post('/api/admin/qrcode', (req, res) => {
  const { qrcodeUrl, qrcodeData } = req.body;
  
  if (!qrcodeUrl && !qrcodeData) {
    return res.status(400).json({ error: '请提供收款码图片URL或Base64数据' });
  }
  ...
  const newQr = {
    id: uuidv4(),
    filename: qrcodeUrl || qrcodeData, // 存储URL或Base64
    uploadTime: Date.now(),
    isActive: true,
    isBase64: !!qrcodeData
  };
